            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <p class="mb-1">Order Date: {{ date('M d, Y', strtotime($order->invoice_date)) }}</p>
                                    <p class="mb-1">Payment Mode: {{ $order->payment_mode }}</p>
                                    <p class="mb-1">Status:
                                        <span class="badge bg-{{ $order->invoice_status === 'Paid' ? 'success' : ($order->invoice_status === 'Pending' ? 'warning' : 'danger') }}">
                            ...
                            <tbody>
                                @foreach ($order->orderDetails as $detail)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <!-- Warehouse Selection Inputs -->
                                            @foreach ($detail->product->inventories as $inventory)
                                                <div class="form-check">
                                                    <label class="form-check-label" for="warehouse-{{ $inventory->id }}">
                                                        {{ $inventory->warehouse->warehouse_name }} (Available: {{ $inventory->remaining }})
                                                    </label>
                                                    <input type="number" wire:model="selectedInventories.{{ $detail->id }}.{{ $inventory->id }}" id="warehouse-{{ $inventory->id }}" 
                                                           class="form-control" min="0" max="{{ $inventory->quantity }}" 
                                                           placeholder="Qty to deliver from warehouse {{ $inventory->warehouse->name }}">
                                                </div>
                                            @endforeach
                                        </td>
                                    </tr>
                    ...
                    <div class="d-flex justify-content-end">
                        <button wire:click="updateDelivery" class="btn btn-success">Update Delivery</button>
                    </div>
                </div>
            </div>
        </div>
